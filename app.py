...
def register_all_routes():
    ...
    def session_password():
        ...
        if request.method == 'POST':
            ...
            if password == current_password:
                ...
                print(f"Session password SET successfully: {session.get('session_authenticated')}")
    ...
    def questionnaire():
        ...
        # Debug logging
        print("=== QUESTIONNAIRE DEBUG INFO ===")
        print(f"Selected language: {selected_language}")
        print(f"Questions data type: {type(questions_data)}")
        print(f"Questions data keys: {questions_data.keys() if isinstance(questions_data, dict) else 'Not a dict'}")
    ...
    def submit_form():
        ...
        if form.validate_on_submit():
            ...
            print(f"Form data received: name={name}, country={country}, gender={gender}")
    ...
    def firebase_config():
        ...
        print(f"Providing API key to frontend: {api_key}")
...
def create_app():
    ...
    try:
        ...
        print(f"--- Loaded API Key: {os.environ.get('FIREBASE_API_KEY', 'Not found')} ---")
        print(f"--- OpenAI API Key loaded: {'Yes' if os.environ.get('OPENAI_API_KEY', '').startswith('sk-') else 'No'} ---")
